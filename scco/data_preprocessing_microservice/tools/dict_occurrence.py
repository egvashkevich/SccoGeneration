...
class DictOccurrenceManager:
    ...
    #TODO:
    # 1) Текст обрабатывать по частям, чтобы очень большие тексты не разбивать полностью, мб поможет, если встречается ближе к началу сообщения
    # 2) Убрать двойные пробелы мб стоит (или нет?)
    # 3) Обобщить общую часть алгоритмов (или нет?)
    # 4) Можно на плюсах суфф автомат написать и быстро искать вхождения (без ошибки точно, с ошибкой уже не строго по ливенштейну, но приставку и суффикс отбросить можно)
